...
hs.ub = java_spinner([59 10 56 22], [p.ub -inf inf p.ub_step], ph, ...
    cb('ub'), p.numFmt, 'max value (clipped)');
    ...
    case 'file'
        ...
        if isfield(p,'numFmt') && ~isempty(p.numFmt)
            try
                ...
                fprintf('[nii_viewer dbg] file-switch setEditor FAILED: %s\n', ME.message);
            end
        else
            fprintf('[nii_viewer dbg] file-switch: p.numFmt MISSING (step=%.6g) — dispPara not applied to this file?\n', p.lb_step);
        end
        nVol = size(p.nii.img, 4);
...
% Pin the spinner step to the SMALLEST nonzero signal value in the overlay
% (= the lowest value in the masked data, since out-of-mask voxels are 0).
% This lets the min/max thresholds be nudged at the finest meaningful
% increment for small-valued overlays (e.g. MRSI ~1e-4).
v = abs(double(p.nii.img(:)));
v = v(isfinite(v) & v > 0);
if isempty(v)
    sigMin = 1;
else
    sigMin = min(v);              % lowest nonzero magnitude
    % To be robust to a single tiny outlier, swap the line above for:
    %   sigMin = prctile(v, 1);   % 1st-percentile of nonzero values
end
p.lb_step = sigMin;
p.ub_step = sigMin;
% Display precision: enough decimals to resolve one step (so it isn't shown as 0.00).
nDec = min(12, max(2, ceil(-log10(sigMin)) + 1));
p.numFmt = ['#.' repmat('#', 1, nDec)];
fprintf('[nii_viewer dbg] dispPara: nz=%d/%d  sigMin=%.6g  nDec=%d  numFmt=%s  lb=%.6g ub=%.6g\n', ...
        numel(v), numel(p.nii.img), sigMin, nDec, p.numFmt, p.lb, p.ub);
...
if ~isfinite(d) || d==0, d = 1; end   % only when val is 0
d = max(d, 1e-9);                       % allow tiny steps for small-valued data
