...
if islogical(cmd)
    ...
    if iscell(outName), outName = outName{1}; end
    outName = char(outName); % R2023a+ gunzip returns string; force char
    return;
end
...
outName = char(outName); % defensive: ensure char on all return paths
...
fname = char(fname); % R2023a+ system() rejects string class
