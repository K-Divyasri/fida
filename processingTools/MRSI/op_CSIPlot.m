    ...
    function [indexs, data] = permuteData(MRSIStruct)
        % Define preferred dimension order (including k-space options)
        dimensionOrder = {'t', 'f', 'y', 'x', 'ky', 'kx', 'coils', 'averages', 'extras'};
    
        % Get dimension indices from struct
        dimNumbers = getMultipleDimensions(MRSIStruct, dimensionOrder);
    
        % Remove zero entries and duplicates
        uniqueDims = unique(dimNumbers(dimNumbers > 0), 'stable');
    
        % Build permutation order
        N = ndims(MRSIStruct.data);
        fullOrder = [uniqueDims, setdiff(1:N, uniqueDims, 'stable')];
    
        % Perform permutation
        data = getData(MRSIStruct);
        data = permute(data, fullOrder);
    
        % Mark which dims were used
        indexs = false(1, N);
        indexs(fullOrder) = true;
    end
